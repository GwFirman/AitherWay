...
import GunungBromo from "@/assets/images/GunungBromo.jpg";
import PantaiKlingking from "@/assets/images/PantaiKlingkingBali.jpg";
					...
					{destination.map((destination, index) => (
						...
						>
							...
							<div className="absolute inset-0">
								<Image src={destination.image} alt={destination.title} className="object-cover" fill sizes="(max-width: 640px) 100vw, (max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" priority={index === 0} />
							</div>

							{/* Rating badge */}
							<div className="absolute top-4 left-4 bg-black/30 backdrop-blur-sm px-3 py-1.5 rounded-full flex items-center z-10">
								<FaStar className="text-yellow-400" />
								<span className="ml-1.5 text-white text-sm font-medium">{destination.rating}</span>
							</div>

							{/* Content overlay */}
							<div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 bg-gradient-to-t from-black/90 via-black/50 to-transparent">
								<h3 className="text-lg sm:text-xl font-semibold mb-2 text-white">{destination.title}</h3>
